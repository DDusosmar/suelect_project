...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['actie'])) {
        if ($_POST['actie'] === 'toevoegen') {
            // Toevoegen
            $voornaam = $suelect_conn->real_escape_string($_POST['voornaam']);
            $achternaam = $suelect_conn->real_escape_string($_POST['achternaam']);
            $type = (int)$_POST['type'];
            ...
            if (!$error) {
                ...
                // Voeg kandidaat toe
                $stmt = $suelect_conn->prepare("INSERT INTO kandidaat (voornaam, achternaam, partij_id, distrikt_id, ressort_id, type, bio, foto_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssiiisss", $voornaam, $achternaam, $partij_id, $distrikt_id, $ressort_id, $type, $bio, $foto_url);
                if ($stmt->execute()) {
...
");
    ...
    while ($row = $result->fetch_assoc()):
        $voornaam_db = htmlspecialchars($row['voornaam']);
        $achternaam_db = htmlspecialchars($row['achternaam']);
        ?>
        <tr>
            ...
            <td>
                ...
                <form method="POST" style="display:inline;">
                    ...
                    <input type="text" name="voornaam" value="<?= htmlspecialchars($voornaam_db) ?>" required>

                    <label>Achternaam:</label>
                    <input type="text" name="achternaam" value="<?= htmlspecialchars($achternaam_db) ?>" required>
...
<table>
    ...
    ");
    while ($row = $result->fetch_assoc()):
        $naam_del = explode(' ', $row['naam'], 2);
        $voornaam_db = $naam_del[0];
        $achternaam_db = $naam_del[1] ?? '';
        ?>
        <tr>
            <td><?= $row['kandidaat_id'] ?></td>
            <td><?= htmlspecialchars($row['naam']) ?></td>
